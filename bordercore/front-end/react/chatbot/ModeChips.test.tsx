...
describe("ModeChips", () => {
  it("renders only chat and notes when no blob context", () => {
    render(<ModeChips mode="chat" hasBlobContext={false} showDjango={false} onChange={vi.fn()} />);
  ...
  it("renders blob chip when hasBlobContext is true", () => {
    render(<ModeChips mode="chat" hasBlobContext={true} showDjango={false} onChange={vi.fn()} />);
    expect(screen.getByRole("button", { name: "blob" })).toBeInTheDocument();
  });

  it("marks the active mode chip as active", () => {
    render(<ModeChips mode="notes" hasBlobContext={false} showDjango={false} onChange={vi.fn()} />);
  ...
  it("calls onChange with the clicked mode", async () => {
    ...
    render(<ModeChips mode="chat" hasBlobContext={false} showDjango={false} onChange={onChange} />);
  ...
  it("renders a non-clickable indicator chip for exercise mode", () => {
    render(
      <ModeChips mode="exercise" hasBlobContext={false} showDjango={false} onChange={vi.fn()} />
    );
    const chip = screen.getByText("exercise");
    expect(chip.tagName).toBe("SPAN");
  });

  it("renders the django chip when showDjango is true", () => {
    render(<ModeChips mode="chat" hasBlobContext={false} showDjango={true} onChange={vi.fn()} />);
    expect(screen.getByRole("button", { name: "django" })).toBeInTheDocument();
  });

  it("hides the django chip when showDjango is false", () => {
    render(<ModeChips mode="chat" hasBlobContext={false} showDjango={false} onChange={vi.fn()} />);
    expect(screen.queryByRole("button", { name: "django" })).not.toBeInTheDocument();
  });
});
