...
export const RelatedTags = forwardRef<RelatedTagsHandle, RelatedTagsProps>(function RelatedTags(
  ...
  const titleSlot = (
    <div className="flex">
      <div className="card-title flex">
        <FontAwesomeIcon icon={faTags} className="text-accent me-4 mt-1" />
        Related Tags
      </div>
    </div>
  );
    ...
    <Card className="backdrop-filter" titleSlot={titleSlot}>
      {tagList.map(tagInfo => (
        <div key={tagInfo.name}>
          ...
          <h5 className="text-ok">{tagInfo.name}</h5>
