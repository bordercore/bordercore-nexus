...
export function SongEditPage({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      formDataToSend.append("original_year", formData.original_year || "");
      formDataToSend.append("rating", formData.rating?.toString() || "");
      formDataToSend.append("note", formData.note || "");
      formDataToSend.append("album_name", formData.album_name || "");
      formDataToSend.append("compilation", formData.compilation ? "true" : "false");
      if (formData.source) {
        formDataToSend.append("source", formData.source.toString());
      }

      // Get tags from the TagsInput component
      const tags = tagsInputRef.current?.getTags() || formData.tags;
      ...
      if (err.response?.data?.errors) {
        ...
        const formErrors: SongFormErrors = {};
        Object.keys(err.response.data).forEach((key) => {
          if (Array.isArray(err.response.data[key])) {
            formErrors[key as keyof SongFormErrors] = err.response.data[key];
          }
        });
  ...
  return (
    <div className="row g-0 h-100 music-dashboard">
      ...
      <div className="col-lg-8 h-100">
        ...
        <form id="song-form" onSubmit={handleSubmit} noValidate>
