...
class Collection(ElasticsearchMixin, TimeStampedModel):
    ...
    def get_recent_images(self, limit: int = 4) -> Any:
        """Return the most recent blob CollectionObjects for thumbnail use.

        Returns up to `limit` blob members of this collection (bookmarks
        excluded), ordered by creation date descending. Every blob has a
        `cover.jpg` generated by the per-blob thumbnail Lambda regardless
        of source type (image, video, PDF, text), so consumers can build
        previews without inspecting file extensions.

        Args:
            limit: Maximum number of objects to return.

        Returns:
            A lazy QuerySet of dicts, each containing "uuid" and "file"
            fields from the most recently created Blobs.
        """
